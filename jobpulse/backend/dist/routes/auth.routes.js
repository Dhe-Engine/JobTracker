...
async function authRoutes(app) {
    ...
    */
    app.get("/google/callback", async (req, reply) => {
        ...
        if (req.query.error) {
            return reply.redirect(`${config_1.config.frontend.url}/?error=access_denied`);
        }
        //step 2: check for missing authorization code
        if (!req.query.code) {
            return reply.redirect(`${config_1.config.frontend.url}/?error=missing_code`);
        }
        try {
    ...
    */
    app.post("/logout", { preHandler: middleware_1.requireAuth }, async (_req, reply) => {
