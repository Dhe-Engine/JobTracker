...
exports.extractCompanyFromSender = extractCompanyFromSender;
exports.classifyEmailBatch = classifyEmailBatch;
...
${bodyPreview}`;
        ...
        */
        ...
        logger_1.logger.error("Gemini email classification failed", {
            error: err,
        });
        return {
 ...
 */
function parseGeminiResponse(rawText, email) {
    /*
    |--------------------------------------------------------------------------
    ...
    */
    try {
        ...
        //   rawText
        // );
        logger_1.logger.error("Failed to parse Gemini response as JSON", {
            rawResponse: rawText,
        });
    }
    /*
    ...
    */
    if (parsed &&
        ...
        const result = {
            is_job_application: parsed.is_job_application,
            company: typeof parsed.company === "string"
                ? parsed.company
                : null,
            role: typeof parsed.role === "string"
                ? parsed.role
                : null,
        ...
        */
        if (!result.is_job_application &&
            ...
            //   }
            // );
            logger_1.logger.info("Email classification overridden by heuristic", {
                subject: email.subject,
                from: email.from,
            });
            return {
    ...
    */
    ...
    // );
    logger_1.logger.warn("Gemini returned unparseable response, using heuristic fallback");
 ...
 */
function isObviousConfirmation(email) {
    /*
    |--------------------------------------------------------------------------
    | Step 1: Check the subject line
    |--------------------------------------------------------------------------
    */
    const subjectMatches = CONFIRMATION_SUBJECT_PATTERNS.some((pattern) => pattern.test(email.subject));
    if (subjectMatches)
        return true;
    /*
    |--------------------------------------------------------------------------
    ...
    */
    ...
    if (emailMatch) {
        ...
        for (const atsDomain of ATS_SENDER_DOMAINS) {
            if (senderDomain.endsWith(`.${atsDomain}`) ||
...
function extractCompanyFromSender(from) {
    ...
    */
    if (ATS_DOMAINS_TO_SKIP.has(domain)) {
        ...
        if (displayNameMatch) {
            ...
            return (displayName
                .replace(/\s+(recruiting|careers|hr|hiring|team|jobs|noreply)$/i, "")
    ...
    */
    ...
    const companyPart = parts.length >= 2
        ? parts[parts.length - 2]
        : parts[0];
    return (companyPart.charAt(0).toUpperCase() +
        companyPart.slice(1));
}
/*
|--------------------------------------------------------------------------
